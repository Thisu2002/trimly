...
export default function ViewStylistModal({
  stylistId,
  open,
  onClose,
}: Props) {
  const [stylist, setStylist] = useState<StylistDetails | null>(null);
    ...
    async function load() {
      ...
      try {
        ...
        const res = await fetch(
          `${apiBase}/api/stylist/${stylistId}?idToken=${token}`
        );
        const data = await res.json();
        if (!res.ok) throw new Error(data.error || "Failed to fetch stylist");
        setStylist(data);
      } catch (err) {
        ...
        {fetching || !stylist ? (
          ...
          <div className="space-y-6">
                ...
                <div className="mt-2 flex flex-wrap items-center gap-3">
                  ...
                  <span className="flex items-center gap-1 text-sm text-gray-300">
                    ...
                    {stylist.yearsOfExperience != null
                      ? `${stylist.yearsOfExperience} yrs exp`
                      : "Stylist"}
                  </span>
                </div>
              </div>
            ...
            <div className="rounded-xl border border-gray-700 bg-[#111827] p-4 space-y-2.5">
              ...
              <InfoRow
                icon={<Phone size={14} />}
                value={stylist.user.phone || "—"}
              />
              <InfoRow
                icon={<MapPin size={14} />}
                value={stylist.user.address || "—"}
              />
            </div>
