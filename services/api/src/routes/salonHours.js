...
router.put("/", async (req, res) => {
    try {
        ...
        await prisma_1.prisma.$transaction(hours.map((item) => prisma_1.prisma.salonBusinessHour.upsert({
            where: {
                salonId_dayOfWeek: {
                    salonId: user.adminSalon.id,
        ...
        const updated = await prisma_1.prisma.salonBusinessHour.findMany({
